...
It uses Groq for high-speed LLM calls and API key rotation
...
1.  `Router`: Use this for:
    * Any question or request related to **data science, AI, machine learning, statistics, or programming**.
    * Any **follow-up command** about the previous message, such as "save that as a PDF", "email it to me", or "generate a quiz for that".
2.  `GeneralChat`: Use this *only* for:
    * Simple greetings (e.g., "hello", "hi", "how are you?").
    * User feedback (e.g., "that was a bad answer", "good job", "you were wrong").
...
def create_agent_node(llm: ChatGroq, tools: list, system_prompt: str): # Updated type hint
...
def define_router_llm(llm: ChatGroq): # Updated type hint
    structured_llm = llm.with_structured_output(Router)
    system_prompt = """You are an expert dispatcher for **ON-TOPIC** data science requests.
                        The user's message has *already* been classified as data science related.
                        ...
                        **CRITICAL ROUTING RULES:**
                        ...
                        2.  **IF THE LAST MESSAGE IS A `HumanMessage` (which you know is on-topic):**
                            * **'KnowledgeBase'**: Route here for questions or content generation (e.g., "explain loss functions", "make a quiz on regression").
                            * **'Researcher'**: Route here for *explicit* web research requests (e.g., "what's the latest news on...", "find me information about...").
                            * **'Writer'**: Route here if the user asks to save, format, or email content (e.g., "save that as a PDF").
                            * **'END'**: Route here if the message is a goodbye (e.g., "thanks", "bye").

                        Choose the *single best* node.
                        """
    prompt = ChatPromptTemplate.from_messages([
        ("system", system_prompt),
        ("placeholder", "{messages}"),
...
def get_agent_graph():
    ...
    GROQ_API_KEYS = get_api_key_from_json(assets_path, "GROQ_API_KEYS")
    if not GROQ_API_KEYS or not isinstance(GROQ_API_KEYS, list) or len(GROQ_API_KEYS) == 0:
        raise ValueError("GROQ_API_KEYS not found in assets.json, is not a list, or is empty.")

    print(f"Creating LLM pool with {len(GROQ_API_KEYS)} Groq keys...")
    llm_pool = [
        ChatGroq(
    ...
    sme_agent = SMEAgent(llm=powerful_llm, assets_path=assets_path)
    ...
    def assignment_generator_tool(query: str) -> str:
        """
        Use this tool to generate text content for an assignment...
        """
    ...
    general_agent_executor = create_agent_node(
        next(llm_cycler),  # Use a fast-rotating LLM
        [feedback_tool],
